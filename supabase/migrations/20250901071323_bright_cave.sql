...
CREATE POLICY "Users can view timeline of accessible applications"
...
CREATE INDEX IF NOT EXISTS idx_applications_approver ON applications(current_approver_user_id);
CREATE INDEX IF NOT EXISTS idx_applications_submitted_at ON applications(submitted_at);
CREATE INDEX IF NOT EXISTS idx_expense_items_application ON expense_items(application_id);
CREATE INDEX IF NOT EXISTS idx_timeline_application ON application_timeline(application_id);
CREATE INDEX IF NOT EXISTS idx_notifications_user ON notifications(user_id);
CREATE INDEX IF NOT EXISTS idx_notifications_read ON notifications(read);
CREATE INDEX IF NOT EXISTS idx_documents_user ON documents(created_by_user_id);
...
-- 新規ユーザー登録時の自動プロファイル作成関数
CREATE OR REPLACE FUNCTION handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (id, full_name, role, plan, status)
  VALUES (NEW.id, NEW.raw_user_meta_data->>'full_name', 'general_user', 'Free', 'active');
